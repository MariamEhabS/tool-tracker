...
/** Props for the GroupActionsBar component -- similar to BulkActionsBar but used specifically for group-level selections. */
export type GroupActionsBarProps = {
  ...
  actions: ReactNode;
  className?: string;
  /** Text label shown after the count. Defaults to "items selected". */
  label?: string;
...
export default function GroupActionsBar(props: GroupActionsBarProps) {
  const {
    selectedCount = 0,
    actions,
    className = "",
  ...
  return (
    <div
      className={`relative bg-gray-100 border border-gray-200 rounded-lg p-3 mb-[15px]  flex items-center justify-between ${transitionClasses} ${className}`}
    >
      {leftContent}
