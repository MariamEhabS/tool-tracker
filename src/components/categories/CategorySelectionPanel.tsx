...
export function CategorySelectionPanel({
  ...
  const { data: companyCategories = [], isLoading } = useCategories(companyId);

  // Filter company categories by search query
  const filteredCategories = useMemo(() => {
    if (!searchQuery.trim()) return companyCategories;
    const query = searchQuery.toLowerCase();
    return companyCategories.filter(
      (c) =>
        c.categoryName.toLowerCase().includes(query) ||
        c.categoryClass.toLowerCase().includes(query),
    );
  }, [companyCategories, searchQuery]);

  // Group company categories by class
  ...
  const hasAnyCategories = companyCategories.length > 0;
        ...
        <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-64 overflow-y-auto">
          {isLoading ? (
            <div className="px-3 py-6 text-center">
              <i className="bx bx-loader-alt bx-spin text-2xl text-gray-400" />
            </div>
          ) : !hasAnyCategories ? (
