...
export const InstructionsPageComponent = ({
  procoreData,
  qrCodeId,
}: InstructionsPageComponentProps) => {
        ...
        <div key={index} className="bg-white rounded-lg">
          <div className="grid grid-cols-3 m-0 mx-auto gap-2 mb-2 px-4 rounded-lg shadow-md bg-gray-100 pb-4 border border-gray-300">
            ...
            <div className="flex flex-col items-start justify-self-start ml-8 col-span-1">
              ...
              <span
                className={`inline-flex items-center px-4 py-0.5 rounded-full text-sm font-medium capitalize ${getStatusColor(data.status || "")}`}
              >
                {data.status || "-"}
              </span>
            </div>
            ...
            <div className="flex flex-col items-start justify-self-start col-span-2">
              ...
              <span className="inline-flex items-center -mt-[3px] rounded-full text-lg">
                {Array.isArray(data.attentions) && data.attentions.length > 0
                  ? data.attentions
                      .map((attention) => attention.name || "-")
                      .join(", ")
                  : "-"}
              </span>
            </div>
            <div className="flex flex-col items-start justify-self-start col-span-2">
              ...
              <span className="inline-flex items-center -mt-[3px] rounded-full text-lg">
                {formatProcoreTime(data.date_issued || "") || "-"}
              </span>
            </div>
            <div className="flex flex-col items-start justify-self-start col-span-3">
              <span className="text-xs text-yellow-900 font-[300]">Type</span>
              <span className="inline-flex items-center -mt-[3px] rounded-full text-lg">
                {data?.instruction_type?.name || "-"}
              </span>
            </div>
                  ...
                  <div>
                    {data?.attachments?.length == 0 ? (
                      <p className="text-sm text-gray-500">
                        No attachments available for this Instruction
                      </p>
                    ) : (
                      <div
                        key={index}
                ...
                {activeTab === "dates" && (
                  <div className="space-y-2">
                    {[
                      ...
                      { label: "Date Received", value: data?.date_received },
                    ].map(({ label, value }) => (
                      <div
                        key={label}
                    ...
                    <div className="flex flex-col justify-start items-start mt-1">
                      ...
                      <p className="font-medium text-gray-900 col-span-1">
                        {Array.isArray(data?.trades) && data.trades.length > 0
                          ? data.trades.map((trade) => trade.name).join(", ")
                          : "-"}
                      </p>
                    </div>
                  </div>
                ...
                {activeTab === "links" && (
                  <div className="space-y-2">
                    <div className="text-sm text-gray-500">
                      No links available for this instruction.
                    </div>
                  </div>
                )}
                {activeTab === "more" && (
                  <div className="space-y-2">
                    <div className="text-sm text-gray-500">
                      Nothing more available for this instruction.
                    </div>
                  </div>
                )}
              </div>
