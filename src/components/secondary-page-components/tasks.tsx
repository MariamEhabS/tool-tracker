...
interface TasksPageComponentProps {
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-4">
      ...
      {procoreData.map((data, index) => (
        <div key={index} className="bg-white rounded-lg">
          <div className="grid grid-cols-3 m-0 mx-auto gap-2 mb-2 px-4 pr-2 rounded-lg shadow-md bg-gray-100 pb-4 border border-gray-300">
            <div className="py-2 pt-4 col-span-2">
            ...
            <div className="flex flex-col items-start justify-self-start col-span-2">
              <span className="text-xs text-yellow-900 font-[300]">Number</span>
            ...
            <div className="flex flex-col items-start justify-self-start col-span-1">
              ...
              <span
                className={`inline-flex items-center ${data?.status && data.status.length > 11 ? "px-2 text-[11px]" : " px-4 text-sm"} py-0.5 rounded-full text-sm font-medium  capitalize ${getStatusColor(data.status || "-")}`}
              >
              ...
              <div className="mt-4">
                ...
                {activeTab === "dates" && (
                  <div className="space-y-2">
                    {[
                      {
                        label: "Date Created",
                        value: formatProcoreTime(data.created_at || ""),
                      },
                ...
                {activeTab === "people" && (
                  <div className="text-sm text-gray-500">
                    No people available for this task.
                  </div>
                )}
                {activeTab === "links" && (
                  <div className="text-sm text-gray-500">
                    No links available for this task.
                  </div>
                )}
                {activeTab === "more" && (
                  <div className="text-sm text-gray-500">
                    Nothing more available for this task.
                  </div>
                )}
              </div>
            </div>
